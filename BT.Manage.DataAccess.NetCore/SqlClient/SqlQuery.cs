...
namespace BT.Manage.DataAccess.SqlClient
{
    public class SqlQuery : IQuery
    {
        ...
        public bool ModelSqlAddCommandText<T>(T data)
        {
            ...
            {
                BT.Manage.Tools.Attributes.TableName tableNameTmp = attr as Tools.Attributes.TableName;
                if (tableNameTmp != null)
                {
                    tableName = tableNameTmp.name;
                }
        ...
        public bool ModelSqlAddCommandText<T>(T data, out bool IsIdentity)
        {
            ...
            for (int i = 0; i < sd.Count(); i++)
            {
                if (sd.GetValue(i).GetType().Name == "TableName")
                {
                    BT.Manage.Tools.Attributes.TableName tableNameTmp = sd[i] as Tools.Attributes.TableName;
                    if (tableNameTmp != null)
                    {
                        tableName = tableNameTmp.name;
                    }
        ...
        public bool ModelSqlAddCommandTextForScope<T>(T data)
        {
            ...
            foreach (PropertyInfo propertyInfo in properties)
            {
                    ...
                    int i = 0; i < attrs.Count(); i++)
                {
                    ...
                    {
                        BT.Manage.Core.RelationTableAttribute k = attrs[i] as BT.Manage.Core.RelationTableAttribute;
